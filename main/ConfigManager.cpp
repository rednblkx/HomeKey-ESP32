...
void ConfigManager::deleteMqttConfig() {
  nvs_erase_key(m_nvsHandle, "MQTTDATA");
  m_mqttConfig = {};
}

void ConfigManager::deleteMiscConfig() {
  nvs_erase_key(m_nvsHandle, "MISCDATA");
  m_miscConfig = {};
}
...
bool ConfigManager::saveMiscConfig(const espConfig::misc_config_t& newConfig) {
    ...
    if (saveConfigToNvs("MISCDATA", newConfig)) {
        ...
        ESP_LOGI(TAG, "Miscellaneous configuration successfully saved and updated.");
        return true;
    }
...
void ConfigManager::loadConfigFromNvs(const char* key, T& configStruct) {
    ...
    if (err == ESP_ERR_NVS_NOT_FOUND) {
        ESP_LOGI(TAG, "Config key '%s' not found in NVS. Using default values.", key);
        // The default values are already set by the struct's constructor
        return;
    } else if (err != ESP_OK) {
        ESP_LOGE(TAG, "Error (%s) getting blob size for key '%s'", esp_err_to_name(err), key);
        return;
    }
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Error (%s) reading blob for key '%s'", esp_err_to_name(err), key);
        return;
    }

    nlohmann::json data = nlohmann::json::from_msgpack(buffer);
    if (!data.is_discarded()) {
        data.get_to<T>(configStruct);
...
bool ConfigManager::saveConfigToNvs(const char* key, const T& configStruct) {
    ...
    std::vector<uint8_t> serialized_data = nlohmann::json::to_msgpack(configStruct);
    
    esp_err_t set_err = nvs_set_blob(m_nvsHandle, key, serialized_data.data(), serialized_data.size());
    if (set_err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to set blob in NVS for key '%s': %s", key, esp_err_to_name(set_err));
        return false;
    }

    esp_err_t commit_err = nvs_commit(m_nvsHandle);
    if (commit_err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to commit NVS changes for key '%s': %s", key, esp_err_to_name(commit_err));
        return false;
    }
