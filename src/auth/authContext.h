...
protected:
  ...
  issuerEndpoints::issuerEndpoints_t *find_endpoint_by_cryptogram(std::vector<uint8_t> cryptogram);
  void Auth1_keying_material(uint8_t *keyingMaterial, const char *context, uint8_t *out, size_t outLen);

public:
  std::tuple<issuerEndpoints::issuerEndpoints_t *, homeKeyReader::KeyFlow> fast_auth(bool fallbackToStd);
  std::tuple<issuerEndpoints::issuerEndpoints_t *, DigitalKeySecureContext, std::vector<uint8_t>, homeKeyReader::KeyFlow> std_auth();
