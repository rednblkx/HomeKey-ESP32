# Prerequisites: Gearing Up for HomeKey-ESP32 🛠️

<!--toc:start-->
- [Prerequisites: Gearing Up for HomeKey-ESP32 🛠️](#prerequisites-gearing-up-for-homekey-esp32-🛠️)
  - [1. Essential Software](#1-essential-software)
    - [1.1. `esptool`](#11-esptool)
      - [Option 1: Command-Line Flashing with esptool.py (Recommended for advanced users)](#option-1-command-line-flashing-with-esptoolpy-recommended-for-advanced-users)
      - [Option 2: Browser-Based Flashing with esptool-js (Recommended for beginners)](#option-2-browser-based-flashing-with-esptool-js-recommended-for-beginners)
    - [1.2. Python 3.x (Only if using esptool.py)](#12-python-3x-only-if-using-esptoolpy)
  - [2. Hardware You'll Need](#2-hardware-youll-need)
    - [2.1. ESP32 Development Board](#21-esp32-development-board)
      - [ESP32 Buyer's Guide](#esp32-buyers-guide)
    - [2.2. PN532 NFC Module](#22-pn532-nfc-module)
      - [Choosing Your PN532 Module: A Mini Buyer's Guide](#choosing-your-pn532-module-a-mini-buyers-guide)
    - [2.3. Integrated PCB Boards - Optional but simpler](#23-integrated-pcb-boards-optional-but-simpler)
<!--toc:end-->
...
#### Option 1: Command-Line Flashing with esptool.py (Recommended for advanced users)
...
#### Option 2: Browser-Based Flashing with esptool-js (Recommended for beginners)
...
*   **How to use it:** You'll access a web page that uses `esptool-js` to flash your ESP32.
    ...
    *   A demo from Espressif is available here: [esptool-js](https://espressif.github.io/esptool-js/)
...
**Important:** Make sure to use a stable power supply. Avoid using a Macbook as they generally don’t supply enough current and can result in unexpected behavior of the ESP32 and/or PN532.
...
Generally, any board should be fine. However, many have non-genuine modules and cheap flash chips with low endurance. There is no real way of telling which is the better clone. Genuine modules typically have "ESPRESSIF" etched on the metal casing. Check reviews.

> [!TIP]
>  Genuine development boards can be ordered from major distributors like Mouser or Digikey.

### 2.2. PN532 NFC Module
...
**Important:** The information given here won’t guarantee that what you buy will be 100% without issues but aims to guide you toward a better part. If buying from AliExpress, suggest checking reviews and photos from customers.
...
You can also buy from Elechouse for higher quality but more expensive parts. They are the original designer of the red boards. You can find their PN532 NFC RFID MODULE V4 on their [official website here](https://www.elechouse.com/product/pn532-nfc-rfid-module-v4/).


### 2.3. Integrated PCB Boards - Optional but simpler 

*   **What they are:** Custom-designed Printed Circuit Boards created by the community. These boards often integrate the ESP32 and PN532, along with other necessary components, into a single, compact solution.
